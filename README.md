...
自用/精选的 Claude Code / Codex 技能仓库，覆盖内容创作、客户服务、市场调研、SEO 优化、Agent 工作流循环设计、Grok X 实时抓取、Claude 无界面评审等场景。
...
| 技能 | 描述 |
|-------|------|
| [skill_auditor](./skill_auditor/) | Claude 技能安全审计工具 |
| [loopforge](./loopforge/) | 把模糊工作流锻造成可维护的 AI Agent 工作流循环：同时输出 `loop-spec-v1` YAML/JSON 和可复制 Loop Prompt |
| [claude-review](./claude-review/) | Codex 驱动 Claude 做无界面评审（plan / 架构 / 文档 / work-log），自动链式调用 `plan-eng-review`。安装到 `~/.codex/skills/` |
| [grok](./grok/) | 让 Claude 调本地 Grok Build CLI 抓 X (Twitter) 实时信息流内容（带 @用户名/互动数/链接的真实帖子）。复用 grok.com 订阅零额外成本。三模式：x 实时抓取 / ask 第二意见 / continue 续接 |
